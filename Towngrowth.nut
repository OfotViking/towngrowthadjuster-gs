...
function Town::AddCargo(){
    for(local i = 0; i < this.econ.num_cargos; i++){
        supply_cargo[i] = 0;
        take_cargo[i] = 0;
        prod_cargo[i] = GSTown.GetLastMonthProduction(this.id, this.econ.enable_order[i]);
...
// except 100% decay (which is easy to calculate)./
	    if(stocked_cargo[i] < 2147483){
		    stocked_cargo[i] *= (1000 - this.econ.decay_rates[i]);
...
function Town::News(){
    // Modify Goal Window
    if(GSController.GetSetting("debug_level") > 3) {GSLog.Info("Town News");}
	this.UpdateGoal();
...
// Add new ones.
	...
	this.addGoalStory(GSText(GSText.STR_CITYBUILDER_GW_DEMANDS_TO));
	this.addGoalStoryEntriesForDemands();
	this.addGoalStory(GSText(GSText.STR_CITYBUILDER_GW_DEMANDS_FROM));	
	this.addGoalStoryEntriesForTakes();
	...
	if(!has_goal) {
	    if(this.pop < tsize) {
	        this.addGoalStory(GSText(GSText.STR_CITYBUILDER_GW_HAMLET));
	    } else {
	        this.addGoalStory(GSText(GSText.STR_CITYBUILDER_GW_VILLAGE));
	    }
	}
	local next = econ.GetNextDemand(this.pop);
	if(next != -1) {
...
function Town::addGoalStoryEntriesForDemands() {
    for(local i = 0; i < this.econ.num_cargos; i++) {
...
function Town::addGoalStoryEntriesForTakes() {
    for(local i = 0; i < this.econ.num_cargos; i++) {
        local cargo_id = this.econ.enable_order[i];
